...
class MPNN_e(hk.Module):
  ...
  def __init__(
    ...
    @jax.jit
    def jax_coo_sum(rows, cols, edge_types, msg_in, msg_out):
#       msg_vect = msg_in[rows][edge_types] + msg_out[cols][edge_types]
      msg_vect = message_stack(msg_in,rows,edge_types) + message_stack(msg_out,cols,edge_types)
      if message_relu:
        msg_vect = jax.nn.relu(msg_vect)
      return jnp.zeros_like(msg_out[:,0,:]).at[rows].add(msg_vect)

    @jax.jit
    def jax_coo_max(rows, cols, edge_types, msg_in, msg_out):
#       msg_vect = msg_in[rows][edge_types] + msg_out[cols][edge_types]
      msg_vect = message_stack(msg_in,rows,edge_types) + message_stack(msg_out,cols,edge_types)
      if message_relu:
        msg_vect = jax.nn.relu(msg_vect)
      return jnp.zeros_like(msg_in[:,0,:]).at[rows].max(msg_vect)

    self.jax_coo_sum = jax_coo_sum
    self.jax_coo_max = jax_coo_max

  def __call__(self, features, rows, cols,edge_types):
    if self.direction == Direction.FORWARD or self.direction == Direction.BOTH:
      m1_1 = hk.Linear(self.mid_size*self.num_edge_types, with_bias=self.with_bias)
#       m1_1 = hk.nets.MLP([self.mid_size] * self.num_edge_types, with_bias=self.with_bias)
      m2_1 = hk.Linear(self.mid_size*self.num_edge_types, with_bias=self.with_bias)
#       m2_1 = hk.nets.MLP([self.mid_size] * self.num_edge_types, with_bias=self.with_bias)
      msg_1_1 = m1_1(features)
      msg_2_1 = m2_1(features)
      print("shape",msg_1_1.shape)
      msg_1_1 = msg_1_1.reshape(-1,self.num_edge_types, self.mid_size)
      msg_2_1 = msg_2_1.reshape(-1,self.num_edge_types, self.mid_size)
    if self.direction == Direction.BACKWARD or self.direction == Direction.BOTH:
      m1_2 = hk.Linear(self.mid_size*self.num_edge_types, with_bias=self.with_bias)
#       m1_2 = hk.nets.MLP([self.mid_size] * self.num_edge_types, with_bias=self.with_bias)
      m2_2 = hk.Linear(self.mid_size*self.num_edge_types, with_bias=self.with_bias)
#       m2_2 = hk.nets.MLP([self.mid_size] * self.num_edge_types, with_bias=self.with_bias)
      msg_1_2 = m1_2(features)
      msg_2_2 = m2_2(features)
      msg_1_2 = msg_1_2.reshape(-1,self.num_edge_types, self.mid_size)
      msg_2_2 = msg_2_2.reshape(-1,self.num_edge_types, self.mid_size)
    ...
    if self.direction == Direction.FORWARD:
      msgs = reduction(rows, cols, edge_types,msg_1_1, msg_2_1)
    elif self.direction == Direction.BACKWARD:
      msgs = reduction(cols, rows, edge_types,msg_1_2, msg_2_2)
    elif self.direction == Direction.BOTH:
      msgs_1 = reduction(rows, cols, edge_types,msg_1_1, msg_2_1)
      msgs_2 = reduction(cols, rows, edge_types,msg_1_2, msg_2_2)
...
class Model_e:
  ...
  def _kl_net(self, features, rows, cols, batch_size, edge_types):
    ...
    for gnn in gnns:
      hidden = gnn(hidden, rows, cols,edge_types)
      hiddens.append(jnp.reshape(hidden, (batch_size, -1, self._num_features)))
